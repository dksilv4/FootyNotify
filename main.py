...
def main():
    ...
    response.message("To look at all the options you can text OPTIONS!")
    for temp in pending:
        if from_no == temp[0]:
    ...
    if msg_body.__contains__("SUBSCRIBE"):
        ...
        last_results = sport.get_last_five_new(team_id)
    ...
    elif msg_body.__contains__("STANDINGS"):
        team_id, team_name = sport.search(msg_body.replace("STANDINGS ", ""))
        response.message(sport.get_standings(team_id))
        return str(response)

    elif msg_body.__contains__("NEXT"):
        team_id, team_name = sport.search(msg_body.replace("NEXT ", ""))
        response.message(sport.get_next_fixture(team_id))
        return str(response)
        ...
        if live_results is None:
            response.message("{} aren't currently playing.".format(team_name))
        else:
            response.message(live_results)
        return str(response)
    elif msg_body.__contains__("OPTIONS"):
        response.message(
            ...
            "\nSTANDING (Team Name) - Replies with the standing for your team in their league. \n"
